...
void lcd_writenibble(unsigned char lcdbits)
{
  ...
  # define MASKBITS 0xf0 //define a mask that isolates the upper 4 bits
  PORTD &= ~MASKBITS; //clear the upper four bits in PORTD
  PORTD |= (lcdbits & MASKBITS); //copy the upper four bits of lcdbits into PORTD
